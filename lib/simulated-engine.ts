...
export async function simulateElizaSignDecision(
  toAddress: string,
  amount: number,
  riskLevel: RiskLevel,
  autoApprove: boolean,
  agentConfig: { autoApproveStatus: boolean; riskThreshold: RiskLevel; signingMethod: string },
): Promise<{
  decision: string
  feedback: string
  ...
  await new Promise((resolve) => setTimeout(resolve, 1000 + Math.random() * 1000)) // Simulate AI processing delay

  let decision = ""
  let feedback = ""
  let type: "success" | "warning" | "error" = "info"
  const chainlinkServicesUsed: string[] = ["Chainlink Functions", "Chainlink Data Feeds"]
  ...
  const transactionRiskScore = riskMapping[riskLevel]
  const agentRiskThresholdScore = riskMapping[agentConfig.riskThreshold]

  if (transactionRiskScore > agentRiskThresholdScore) {
    decision = "❌ Tx Blocked"
    feedback = `Tx Blocked (High Risk Detected: Transaction risk score ${transactionRiskScore} exceeds agent threshold ${agentRiskThresholdScore}).`
    type = "error"
    chainlinkServicesUsed.push("Chainlink Automation (Block)")
  } else if (transactionRiskScore <= agentRiskThresholdScore && autoApprove && agentConfig.autoApproveStatus) {
    decision = "✅ Auto-Signed"
    feedback = `Tx Auto-Signed (Low Risk – Auto-approve enabled. Agent approved transaction automatically).`
    type = "success"
    chainlinkServicesUsed.push("Chainlink Automation (Auto-Sign)")
  } else {
    decision = "✅ Delegated Signing"
    feedback = `Tx Approved by Agent (Medium Risk – Delegated Mode. Agent approved, but required delegation due to settings or moderate risk).`
    type = "warning"
    chainlinkServicesUsed.push("Chainlink Automation (Delegate)")
  }

  return { decision, feedback, type, chainlinkServicesUsed }
}
