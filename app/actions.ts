...
import { simulateElizaSignDecision, type RiskLevel } from "@/lib/simulated-engine"
...
export async function getElizaAIResponse(options: ElizaAIOptions): Promise<{
  ...
  suggestedAction?: string // For ElizaSign AI specific actions
  elizaSignDecision?: {
    decision: string
    feedback: string
  ...
  if (lowerMessage.includes("simulate transaction") && toAddress && amount && riskLevel && agentConfig) {
    const result = await simulateElizaSignDecision(toAddress, amount, riskLevel, autoApprove || false, agentConfig)
    responseText = `Transaction simulation complete! Eliza Agent's decision: **${result.decision}**. \n\n**Feedback:** ${result.feedback}`
    elizaSignDecisionResult = result
  ...
  **Powered by Chainlink:**
  ...
  return { responseText, actionToTrigger, elizaSignDecision: elizaSignDecisionResult }
}
