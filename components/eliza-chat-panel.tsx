...
interface ElizaChatPanelProps {
  ...
  // New props for ElizaSign AI
  onSimulateTransaction?: (toAddress: string, amount: number, riskLevel: RiskLevel, autoApprove: boolean) => void
  agentConfig: { autoApproveStatus: boolean; riskThreshold: RiskLevel; signingMethod: string }
}
...
export const ElizaChatPanel: React.FC<ElizaChatPanelProps> = ({
  ...
  getElizaAIResponse,
  systemActionMessage,
  onSimulateTransaction, // New prop
  agentConfig, // New prop
}) => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      type: "eliza",
      content:
        "Hi! I'm Eliza, your AI DeFi assistant for Smart Contract Signing. I can help you simulate risk-managed transactions. Just tell me what you'd like to do!",
      timestamp: new Date(),
    },
  ])
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await getElizaAIResponse({
        ...
        agentConfig, // Pass agentConfig to AI
      })

      const elizaMessage: ChatMessage = {
        id: `eliza-${Date.now()}`,
        type: "eliza",
        content: response.responseText || "I'm processing your request...",
        timestamp: new Date(),
      }
  ...
  const handleQuickAction = (action: string) => {
    // For ElizaSign AI, quick actions might trigger specific simulations
    if (action.startsWith("Simulate ")) {
      const parts = action.split(" ")
      const risk = parts[1].toLowerCase() as RiskLevel
      // Example values for toAddress and amount for quick actions
      const toAddress = "0xSimulatedAddress" + Math.random().toString(16).slice(2, 6)
      const amount = Number.parseFloat((Math.random() * 100 + 1).toFixed(2))

      if (onSimulateTransaction) {
        onSimulateTransaction(toAddress, amount, risk, agentConfig.autoApproveStatus)
      }
      const actionMessage: ChatMessage = {
        id: `action-${Date.now()}`,
        type: "user",
        content: `Simulate: ${risk} Risk Transaction`,
        timestamp: new Date(),
      }
